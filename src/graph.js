$('#periods').change(function () {
    if ($('#periods').val() == '1') {
        ...
        fark_t2_t1 = math.multiply(100, math.subtract(listDengCoor_t2_t1, listDengCoor_t1)); //convert cm.
        ...
        mtrx_xBil_t2_t1 = matrixZeros_list_xBil_t2; //in matrix format.
        ...
        $("#GlobalTest_t2t1").click(function () {
            ...
            if (T_Test_t2t1 > F_Value_t2t1) {
                ...
                Swal.fire({
                    title: 'T < F',
                    html: 'There is no deformation at the conjugate points. <p style="font-size:16px;"><br>"T" Test Value = ' + T_Test_t2t1 + '<br> "F" Limit Value = ' + F_Value_t2t1 + '</br></p>',
                    icon: 'success',
                    confirmButtonText: 'Ok'
                });
            }

            //-----Calculate Rmin (In Case T>F)
            $("#DatumNokta_t2t1").click(function () {
                let index_Rmin = []; //Rmin_indexleri (the values here indicate the order of points)
                for (var ab = 0; ab < NoktaSayisi_t1; ab++) {
                    ...
                    for (var i = 0; i < NoktaSayisi_t1; i++) {
                        ...
                        if (i > 0) {
                            for (var k = i - 1; k >= 0; k--) {
                                //R value is calculated, GT values are printed on the remaining points.
                        ...
                        //"xi" Minimized Unknowns and "Qi" Cofactor Matrix of Unknowns ( minimally constrained adjustments (kısmi-iz in Tukish))
                        ...
                        let mtrxKismiDeng_t1 = math.add(matrix1tumCoor_t1, mtrx_xi_t1); //in matrix format.
                        ...
                        mtrx_list_Coor_t2_t1 = matrixZeroslist_Coor_t2; //in matrix format.

                        let mtrxKismiDeng_t2 = math.add(mtrx_list_Coor_t2_t1, mtrx_xi_t2); //in matrix format.
                        //---------------
                        //
                        ...
                        let matrix_x_t1_EslenikNokta = matrix_t1_GlobalTest;
                        let matrix_x_t2_EslenikNokta = matrix_t2_GlobalTest;
                        //---

                        const rows_Q_t1_GlobalTest = list_x_t1_GlobalTest.length;
                        const columns_Q_t1_GlobalTest = list_x_t1_GlobalTest.length;
                        matrix_Qti_1_EslenikNokta = math.zeros(rows_Q_t1_GlobalTest, columns_Q_t1_GlobalTest);
                        matrix_Qti_2_EslenikNokta = math.zeros(rows_Q_t1_GlobalTest, columns_Q_t1_GlobalTest);
                        ...
                        matrix_GT_t2t1_EslenikNokta = math.zeros(rows_GT_t2t1_GlobalTest, columns_GT_t2t1_GlobalTest);
                        ...
                        if (row === 0) {
                            ...
                            for (j = 0; j < 2; j++) {
                                if (j === 0) {
                                    ...
                                    td = document.createElement('td');
                                    td.innerHTML = static_points_name_t1t2[(row - 1)]; //static point name
                                    tr.appendChild(td);
                                }
                            }
                            table_swal_t2t1.appendChild(tr);
        ...
        $("#GlobalTest_t3t1").click(function () {
            ...
            if (T_Test_t3t1 > F_Value_t3t1) {
                ...
                Swal.fire({
                    title: 'T < F',
                    html: 'There is no deformation at the conjugate points. <p style="font-size:16px;"><br>"T" Test Value = ' + T_Test_t3t1 + '<br> "F" Limit Value = ' + F_Value_t3t1 + '</br></p>',
                    icon: 'success',
                    confirmButtonText: 'Ok'
                });
            }
            ...
            $("#DatumNokta_t3t1").click(function () {
                ...
                for (var ab = 0; ab < NoktaSayisi_t1; ab++) {
                    ...
                    for (var i = 0; i < NoktaSayisi_t1; i++) {
                        ...
                        mtrx_list_Coor_t3_t1 = matrixZeroslist_Coor_t3; //in matrix format.

                        let mtrxKismiDeng_t3 = math.add(mtrx_list_Coor_t3_t1, mtrx_xi_t3); //in matrix format.
                        //---------------
                        //
                        ...
                        matrix_x_t1_EslenikNokta = matrix_t1_GlobalTest;
                        matrix_x_t3_EslenikNokta = matrix_t3_GlobalTest;
                        //---

                        let rows_Q_t1_GlobalTest = list_x_t1_GlobalTest.length;
                        let columns_Q_t1_GlobalTest = list_x_t1_GlobalTest.length;
                        let matrix_Qti_1_EslenikNokta = math.zeros(rows_Q_t1_GlobalTest, columns_Q_t1_GlobalTest);
                        let matrix_Qti_3_EslenikNokta = math.zeros(rows_Q_t1_GlobalTest, columns_Q_t1_GlobalTest);
                        ...
                        let h_t3i_t1i = math.subtract(math.multiply(datum_nokta_sayisi_t3t1, 3), 3);
                ...
                if (index_Rmin.length === (NoktaSayisi_t1 - 1)) {
                    ...
                    for (let row = 0; row < (static_points_name_t1t3.length + 1); row++) { //+1
                        tr = document.createElement('tr');
                        if (row === 0) {
                            for (var column1 = 0; column1 < 2; column1++) {
                                if (column1 === 0) {
                            ...
                            for (let j = 0; j < 2; j++) {
                                if (j === 0) {
                                    ...
                                    td = document.createElement('td');
                                    td.innerHTML = static_points_name_t1t3[(row - 1)];
                                    tr.appendChild(td);
                                }
                            }
                            table_swal_t3t1.appendChild(tr);
        ...
        mtrx_xBil_t3_t2 = matrixZeros_list_xBil_t3_2; //in matrix format.
        ...
        $("#GlobalTest_t3t2").click(function () {
            ...
            if (T_Test_t3t2 > F_Value_t3t2) {
                ...
                Swal.fire({
                    title: 'T < F',
                    html: 'There is no deformation at the conjugate points. <p style="font-size:16px;"><br>"T" Test Value = ' + T_Test_t3t2 + '<br> "F" Limit Value = ' + F_Value_t3t2 + '</br></p>',
                    icon: 'success',
                    confirmButtonText: 'Ok'
                });
            }
            ...
            $("#DatumNokta_t3t2").click(function () {
                ...
                for (var ab = 0; ab < NoktaSayisi_t2; ab++) {
                    ...
                    for (var i = 0; i < NoktaSayisi_t2; i++) {
                        ...
                        mtrx_list_Coor_t3_t2 = matrixZeroslist_Coor_t3_2;

                        let mtrxKismiDeng_t3 = math.add(mtrx_list_Coor_t3_t2, mtrx_xi_t3);
                        //---------------
                        //
                        ...
                        let matrix_x_t2_EslenikNokta = matrix_t2_GlobalTest;
                        let matrix_x_t3_EslenikNokta = matrix_t3_GlobalTest;
                        //---

                        let rows_Q_t2_GlobalTest = list_x_t2_GlobalTest.length;
                        let columns_Q_t2_GlobalTest = list_x_t2_GlobalTest.length;
                        let matrix_Qti_2_EslenikNokta = math.zeros(rows_Q_t2_GlobalTest, columns_Q_t2_GlobalTest);
                        let matrix_Qti_3_EslenikNokta = math.zeros(rows_Q_t2_GlobalTest, columns_Q_t2_GlobalTest);
                        ...
                        matrix_GT_t3t2_EslenikNokta = math.zeros(rows_GT_t3t2_GlobalTest, columns_GT_t3t2_GlobalTest);
                        ...
                        let h_t3i_t2i = math.subtract(math.multiply(datum_nokta_sayisi_t3t2, 3), 3);
                        ...
                        if (row === 0) {
                            ...
                            for (let j = 0; j < 2; j++) {
                                if (j === 0) {
                                    ...
                                    td = document.createElement('td');
                                    td.innerHTML = static_points_name_t2t3[(row - 1)]; //static point name.
                                    tr.appendChild(td);
                                }
                            }
                            table_swal_t3t2.appendChild(tr);
