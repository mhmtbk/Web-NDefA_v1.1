...
$("#DeformationTest_t2t1").click(function () {
    ...
    for (let i = static_point_count_t1t2 - 1; i >= 0; i--) {
        Noktalar_t1_sgnf.splice(static_points_name_index_t2t1_t1[i], 1); //Static nokta harici T1 Nokta adları
        Noktalar_t2_sgnf.splice(static_points_name_index_t2t1_t2[i], 1); //Static nokta harici T2 Nokta adları
    };
    //
...
$("#Velocity_t2t1").click(function () {
    ...
    VectorDist_t2t1 = []; //metre cinsinden
    ...
    //--- Vertical Displacement Vector (Elipsoit Üzerinde 1. Temel Ödev)
    ...
    for (i = 0; i < (new_point_count_t2t1); i++) {
        ...
        var A_Elps_t2t1 = tumSo_t1 * math.sqrt(lamda1); //Birimi m
        var B_Elps_t2t1 = tumSo_t1 * math.sqrt(lamda2); //Birimi m

        var Elips_YarıEksen_Carpani = GenisletmeKatsayisi_t2t1; //A ve B yarı eksen uzunlukları GenisletmeKatsayisi_t2t1 ile genişletilir.
