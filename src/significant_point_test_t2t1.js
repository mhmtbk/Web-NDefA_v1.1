...
$("#DeformationTest_t2t1").click(function () {
    ...
    for (let i = static_point_count_t1t2 - 1; i >= 0; i--) {
        Noktalar_t1_sgnf.splice(static_points_name_index_t2t1_t1[i], 1); //T1 stations names (except static points).
        Noktalar_t2_sgnf.splice(static_points_name_index_t2t1_t2[i], 1); //T2 stations names (except static points).
    };
    //


    //Point coordinates found with free network adjustment (static points not included)
    matrix1tumCoor_t1_sgnf = math.zeros((NoktaSayisi_t1 - static_point_count_t1t2) * 3, 1) //Raw Coordinates-T1 (except static point)
    ...
    matrix1tumCoor_t2_sgnf = math.zeros((NoktaSayisi_t2 - static_point_count_t1t2) * 3, 1) //Raw Coordinates-T2 (except static point)
    for (let i = 0; i < (NoktaSayisi_t2 - static_point_count_t1t2); i++) {
    ...
    new_point_count_t2t1 = (NoktaSayisi_t2 - static_point_count_t1t2); //Number of points (except static point)

    TumIzMtrx1A_t1_sgnf = math.zeros(BazSayisi_t1 * 3, new_point_count_t2t1 * 3) //A matrix-T1 (except static point)
    ...
    TumIzMtrx1A_t2_sgnf = math.zeros(BazSayisi_t2 * 3, new_point_count_t2t1 * 3) //A matrix-T2 (except static point)
    for (let i = 0; i < new_point_count_t2t1; i++) {
    ...
    n_sgnf_t1 = BazSayisi_t1 * 3; //Measured baseline numbers.
    u_sgnf_t1 = (NoktaSayisi_t1 - static_point_count_t1t2) * 3; //The number of unknown points after static points are removed.
    f_sgnf_t1 = n_sgnf_t1 - u_sgnf_t1; //Degrees of freedom
    ...
    //---Match Point Names in Period 2 by Period 1 and Get Coordinates of Matching Points
    ...
    //φ (Calculation of latitude) (t2) (Partial Points)
    ...
    //Arrangement of the cofactor matrix in Period 2 according to Period 1 (matrix_Qt2t1_sgnf)
...
$("#Velocity_t2t1").click(function () {
    ...
    VectorDist_t2t1 = []; //in meters
    VectorDist_Fixed_t2t1 = [];
    for (var i = 0; i < new_point_count_t2t1; i++) {
        var distSaga_t2t1 = Math.pow(mtrx_SagaYukariFark_t2t1_Kismi.subset(math.index(i, 0)), 2); //in meters
        var distYukari_t2t1 = Math.pow(mtrx_SagaYukariFark_t2t1_Kismi.subset(math.index(i, 1)), 2); //in meters

        var VectorDist_t2t1_a = math.sqrt(distSaga_t2t1 + distYukari_t2t1);
        VectorDist_t2t1.push(VectorDist_t2t1_a); //in meters
    ...
    for (var i = 0; i < new_point_count_t2t1; i++) {
        ...
        GenisletmeKatsayisi_t2t1 = 450000; //Value found by expanding VectorDist_t2t1 by 100 to convert to meters
    ...
    //---Vertical Displacement Vector t2t1
    ...
    //--- Horizontal Displacement Vector t2t1 ---
    layerGroup_t2t1 = L.layerGroup().addTo(map);
    for (var i = 0; i < new_point_count_t2t1; i++) {
    ...
    for (i = 0; i < (new_point_count_t2t1); i++) {
        ...
        var teta_Elips_t2t1_Ham = (1 / 2) * Math.atan(pay_teta / payda_teta) * ro; //Orientation Angle
        ...
        var A_Elps_t2t1 = tumSo_t1 * math.sqrt(lamda1); //unit meter
        var B_Elps_t2t1 = tumSo_t1 * math.sqrt(lamda2); //unit meter

        var Elips_YarıEksen_Carpani = GenisletmeKatsayisi_t2t1; //A and B half-axis lengths are expanded with  GenisletmeKatsayisi_t2t1.
