function doOpen_t3(evt) {
    ...
    else
    {
        ...
        // ------------ Get rows with "@+ and @-" in them (Get rows of referance (@+) and rover (@-) stations in Baselines.)
        var listBazRaw = [];
        for (i = 0; i < LGO_txt.length; i++) {
        ...
        // ------------ Deleting spaces in rows containing "@+ ve @-".
        var listbaz1 = [];
        var listbaz2 = [];
        ...
        Noktalar_t3_t3t1 = bazNN.filter(onlyUnique); //Stations names to be used in T3T1 analysis (for Sgnf)
        Noktalar_t3_t3t2 = bazNN.filter(onlyUnique); //Stations names to be used in T3T2 analysis (for Sgnf)
        ...
        for (var j = 0; j < BazDent_Points_t3.length; j++) {
            ...
            //Geometric parameters of the GRS80 ellipsoids
            a_XYZ = 6378137.0;
            b_XYZ = 6356752.3141;
        ...
        // ------------------- Baseline Vectors
        const rowstumEnlem_Boylam = BazDent_Points_t3.length; //Points Number
        ...
        NoktaSayisi_t3 = bazNNunique_t3.length; //Total Number of Points.
        BazSayisi_t3 = (baz.length) / 2; //Total Number of Baselines.
        // ------------
        ...
        $("#tumizdeng_3").click(function () {
            ...
            matrix1tumCoor_t3NN = matrixZerostumCoorNN; //Display of point names in matrix format in 1 column *****
            ...
            matrix1tumCoor_t3 = matrixZerostumCoor; //Displaying the coordinates of the points in matrix format in 1 column *****

            mtrxDengtum_1_t3 = math.add(matrix1tumCoor_t3, mtrx1tum_x_t3); //Display of adjusted coordinates in 1 column. *****

            mtrxDengtum1_1 = math.concat(matrix1tumCoor_t3NN, mtrxDengtum_1_t3) //Display of adjusted coordinates in 2 columns (SN and Coordinates)

            mtrxDengtum1_1_t3x = math.concat(matrix1tumCoor_t3NN, mtrxDengtum_1_t3, mtrx1tum_x_t3) //Displaying NN, Coordinates and Unknowns in 3 columns *****
            ...
            mtrxtumDengOlc1 = math.add(matrix1Coorbaz, mtrx1tum_V); //Adjusted measures. *****
            ...
            var BaztumREF = listtumREF //Adjusted coordinates of the referance point in the baselines.
            ...
            let mtrxtumBazDengCoor = matrixZerostumBazDengCoor; //Measures in matrix format from Adjusted Coordinate differences, 1 column. *****
            ...
            matrix1tumbazNN_t3 = matrixZerostumbazNN; //Display of baseline points in ∆X, ∆Y, ∆Z format in 1 column in matrix form *****
            ...
            // ------------ Degrees of Freedom
            n_tumiz = BazSayisi_t3 * 3; //Measured baseline numbers.
            u_tumiz = NoktaSayisi_t3 * 3; //Coordinates of points whose coordinates are unknown.
            d_tumiz = 3; // Defekt number.
            f_tumiz_t3 = n_tumiz - u_tumiz + d_tumiz; //Degrees of freedom. If f > 0, adjustment process can be done.
            // ------------

            mtrxtumSo_t3 = math.sqrt(math.divide((math.multiply(math.transpose(mtrx1tum_V), mtrx1P_t3, mtrx1tum_V)), f_tumiz_t3))
            tumSo_t3 = mtrxtumSo_t3.get([0, 0]); //Mean Square Error.
            ...
            for (i = 0; i < NoktaSayisi_t3 * 3; i++) {
                matrixZerostumUnknownPointsCof.subset(math.index(i, 0), math.sqrt(mtrx1tum_QX_t3.get([i, i]))); //square root of cofactors.
            }
            ...
            mtrx1tumKOH_ = math.concat(matrix1tumCoor_t3NN, mtrx1tumUnknownPointsOrtHata) //Mean errors of Unknowns and Point Names, 2 columns.
            ...
            mtrx1t3tumDeng_KOH = math.concat(mtrx1tumDeng, mtrx1tumKOH) //Adjusted coordinates and mean errors of Unknowns and SN, 7 columns. *****
            // ----------
            // -------------------
            ...
            mtrx_TumDeng_KOH_EnlemBoylam_t3 = math.concat(mtrx1t3tumDeng_KOH, matrixTumDeng_EnlemBoylam_t3, matrixTumDeng_DoguKuzey_t3) //NN, Adjusted coordinates of unknowns, mean errors, latitude-longitude; 12 columns with east-north values. (Matrix Form)
