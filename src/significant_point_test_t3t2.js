...
$("#DeformationTest_t3t2").click(function () {
    ...
    for (let i = static_point_count_t2t3 - 1; i >= 0; i--) {
        Noktalar_t2_t3t2_sgnf.splice(static_points_name_index_t2t3_t2[i], 1); //T2 stations names (except static points).
        Noktalar_t3_t3t2_sgnf.splice(static_points_name_index_t2t3_t3[i], 1); //T3 stations names (except static points).
    };
    //

    //Point coordinates found with free network adjustment (static points not included)
    matrix1tumCoor_t2_t3t2_sgnf = math.zeros((NoktaSayisi_t2 - static_point_count_t2t3) * 3, 1) //Raw Coordinates-T2 (except static point)
    ...
    matrix1tumCoor_t3_t3t2_sgnf = math.zeros((NoktaSayisi_t3 - static_point_count_t2t3) * 3, 1) ///Raw Coordinates-T3 (except static point)
    ...
    new_point_count_t3t2 = (NoktaSayisi_t3 - static_point_count_t2t3); //Number of points (except static point)

    TumIzMtrx1A_t2_t3t2_sgnf = math.zeros(BazSayisi_t2 * 3, new_point_count_t3t2 * 3) //A matrix-T2 (except static point)
    ...
    TumIzMtrx1A_t3_t3t2_sgnf = math.zeros(BazSayisi_t3 * 3, new_point_count_t3t2 * 3) //A matrix-T3 (except static point)
    for (let i = 0; i < new_point_count_t3t2; i++) {
    ...
    n_sgnf_t2_t3t2 = BazSayisi_t2 * 3; //Measured baseline numbers.
    u_sgnf_t2_t3t2 = (NoktaSayisi_t2 - static_point_count_t2t3) * 3; //The number of unknown points after static points are removed.
    f_sgnf_t2_t3t2 = n_sgnf_t2_t3t2 - u_sgnf_t2_t3t2; //Degrees of freedom

    n_sgnf_t3_t3t2 = BazSayisi_t3 * 3; //Measured baseline points.
    u_sgnf_t3_t3t2 = (NoktaSayisi_t3 - static_point_count_t2t3) * 3; //Number of unknown points (after static points are removed)
    f_sgnf_t3_t3t2 = n_sgnf_t3_t3t2 - u_sgnf_t3_t3t2; //Degrees of freedom
    ...
    //--- Match Point Names in Period 3 by Period 2 and Get Coordinates of Matching Points
    ...
    //Arrangement of the cofactor matrix in Period 3 according to Period 2 (matrix_Qt3t2_sgnf)
...
$("#Velocity_t3t2").click(function () {
    ...
    VectorDist_t3t2 = [];//in meters
    VectorDist_Fixed_t3t2 = [];
    for (var i = 0; i < new_point_count_t3t2; i++) {
        var distSaga_t3t2 = Math.pow(mtrx_SagaYukariFark_t3t2_Kismi.subset(math.index(i, 0)), 2); //in meters
        var distYukari_t3t2 = Math.pow(mtrx_SagaYukariFark_t3t2_Kismi.subset(math.index(i, 1)), 2); //in meters

        var VectorDist_t3t2_a = math.sqrt(distSaga_t3t2 + distYukari_t3t2);
        VectorDist_t3t2.push(VectorDist_t3t2_a); //in meters
    ...
    for (var i = 0; i < new_point_count_t3t2; i++) {
        ...
        GenisletmeKatsayisi_t3t2 = 450000; //Value found by expanding VectorDist_t3t2 by 100 to convert to meters
        P1PNewPoint_Vertical_t3t2 = (GenisletmeKatsayisi_t3t2 * h_elipsoit_fark_kismi_t3t2[i])
    ...
    //---Vertical Displacement Vector 3t2
    ...
    for (var i = 0; i < new_point_count_t3t2; i++) {
        GenisletmeKatsayisi_t3t2 = 450000;
        P1PNewPoint_t3t2 = (GenisletmeKatsayisi_t3t2 * VectorDist_t3t2[i])
    ...
    //---Horizontal Displacement Vector t3t2 ---
    layerGroup_t3t2 = L.layerGroup().addTo(map);
    for (var i = 0; i < (new_point_count_t3t2); i++) {
    ...
    for (i = 0; i < (NoktaSayisi_t2 - static_point_count_t2t3); i++) {
        ...
        var teta_Elips_t3t2_Ham = (1 / 2) * Math.atan(pay_teta / payda_teta) * ro; //Orientation Angle
        ...
        var A_Elps_t3t2 = tumSo_t2 * math.sqrt(lamda1); //unit meter
        var B_Elps_t3t2 = tumSo_t2 * math.sqrt(lamda2); //unit meter

        var Elips_YarıEksen_Carpani = GenisletmeKatsayisi_t3t2; //A and B half-axis lengths are expanded with  GenisletmeKatsayisi_t3t2.
