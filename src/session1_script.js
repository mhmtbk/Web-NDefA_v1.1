function doOpen(evt) {
    ...
    reader.addEventListener("load", function () {
        ...
        //Choose Label
    var a_123 = document.getElementById('openselect');
    if(a_123.value == "")
    {
    ...
    {
        var theSplit = a_123.value.split('\\');
        ...
        Noktalar_t1 = bazNN.filter(onlyUnique); //Station names, T1 (for Sgnf)
        Noktalar_t1_t3t1 = bazNN.filter(onlyUnique); //Stations names to be used in T3T1 analysis (for Sgnf)
        ...
        $("#tumizdeng").click(function () {
            ...
            matrix1tumCoor_t1NN = matrixZerostumCoorNN; //Display of point names in matrix format in 1 column *****
            ...
            matrix1tumCoor_t1 = matrixZerostumCoor; //Displaying the coordinates of the points in matrix format in 1 column *****

            mtrxDengtum_1_t1 = math.add(matrix1tumCoor_t1, mtrx1tum_x_t1); //Display of adjusted coordinates in 1 column. *****

            mtrxDengtum1_1 = math.concat(matrix1tumCoor_t1NN, mtrxDengtum_1_t1) //Display of adjusted coordinates in 2 columns (SN and Coordinates)

            mtrxDengtum1_1_t1x = math.concat(matrix1tumCoor_t1NN, mtrxDengtum_1_t1, mtrx1tum_x_t1) //Displaying NN, Coordinates and Unknowns in 3 columns *****
            ...
            matrix1tumbazNN = matrixZerostumbazNN; //Display of baseline points in ∆X, ∆Y, ∆Z format in 1 column in matrix form *****
            ...
            // ------------ Degrees of Freedom
            n_tumiz = BazSayisi_t1 * 3; //Measured baseline numbers.
            u_tumiz = NoktaSayisi_t1 * 3; //Coordinates of points whose coordinates are unknown.
            d_tumiz = 3; //Defect number.
            f_tumiz_t1 = n_tumiz - u_tumiz + d_tumiz; //Degrees of freedom. If f > 0, adjustment process can be done.
            // ------------

            mtrxtumSo_t1 = math.sqrt(math.divide((math.multiply(math.transpose(mtrx1tum_V), mtrx1P_t1, mtrx1tum_V)), f_tumiz_t1))
            tumSo_t1 = mtrxtumSo_t1.get([0, 0]); //Mean Square Error.
            ...
            for (i = 0; i < NoktaSayisi_t1 * 3; i++) {
                matrixZerostumUnknownPointsCof.subset(math.index(i, 0), math.sqrt(mtrx1tum_QX.get([i, i]))); //square root of cofactors.
            }
            ...
            mtrx1tumKOH_ = math.concat(matrix1tumCoor_t1NN, mtrx1tumUnknownPointsOrtHata) //Mean errors of Unknowns and Point Names, 2 columns.
            ...
            mtrx1t1tumDeng_KOH = math.concat(mtrx1tumDeng, mtrx1tumKOH) //Adjusted coordinates and mean errors of Unknowns and SN, 7 columns. *****
            // ----------
            // -------------------

            // Point Names on the X-Axis
            ...
            mtrx_TumDeng_KOH_EnlemBoylam_t1 = math.concat(mtrx1t1tumDeng_KOH, matrixTumDeng_EnlemBoylam_t1, matrixTumDeng_DoguKuzey_t1) //NN, Adjusted coordinates of unknowns, mean errors, latitude-longitude; 12 columns with east-north values. (Matrix Form)
            ...
            for (i = 0; i < (BazSayisi_t1 * 3); i++) {
                if (mtrxTersDelta0li_t1.subset(math.index(i)) < math.multiply(mtrxSi_t1.subset(math.index(i)), 8)) {
                    ...
                    matrixZeros_IcGuven_t1.subset(math.index(i, 1), "Ok"); //"Interior Reliability Criteria value is good."
                } else {
                    matrixZeros_IcGuven_t1.subset(math.index(i, 0), mtrxTersDelta0li_t1.subset(math.index(i)));
            ...
            for (i = 0; i < (BazSayisi_t1 * 3); i++) {
                if (mtrxDelta0li_t1.subset(math.index(i)) < 10) {
                    ...
                    matrixZeros_DisGuven_t1.subset(math.index(i, 1), "Ok"); //"Exterior Reliability Criteria value is good."
                } else {
                    matrixZeros_DisGuven_t1.subset(math.index(i, 0), mtrxDelta0li_t1.subset(math.index(i)));
